...
def read_operations_in_time_range(after: datetime, before: datetime, db: Session = Depends(get_db)):
    operations = operation_service.get_operations_in_time_range(after=after,
                                                                before=before,
                                                                db=db)
    if not operations:
        raise HTTPException(status_code=404, detail="No operation found")
...
@router.put("/")
def update_operation(operation: schemas.Operation,
