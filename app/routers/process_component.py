...
from .. import schemas
from ..services import process_component_service, process_service
...
def read_process_components_by_component_id(component_id: str, db: Session = Depends(get_db)):
    process_components = process_component_service.get_process_components_by_component_id(
...
def create_process_component(process_component: schemas.ProcessComponentCreate, db: Session = Depends(get_db)):
    return process_component_service.create_process_component(process_component=process_component, db=db)


@router.put("/")
def update_process_component(process_component: schemas.ProcessComponent,
                             db: Session = Depends(get_db)):
    db_process_component_data = process_component_service.get_process_component(process_component.id, db=db)
    if not db_process_component_data:
        raise HTTPException(status_code=400, detail="Matching process component not found")
    db_process_component_model = schemas.ProcessComponent(**jsonable_encoder(db_process_component_data))
    ...
    return process_component_service.update_process_component(process_component=updated_process_component, db=db)


@router.delete("/{process_component_id}")
def delete_process_component(process_component_id: int, db: Session = Depends(get_db)):
    db_process_component_data = process_component_service.get_process_component(process_component_id, db=db)
    if not db_process_component_data:
        raise HTTPException(status_code=400, detail="Matching process component not found")
    process_component_service.delete_process_component(process_component=db_process_component_data, db=db)
