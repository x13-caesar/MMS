...
def get_employee_by_phone(phone: str, db: Session):
    return db.query(models.Employee).filter(models.Employee.phone == phone).all()
...
def get_employees_in_birthday_range(after: datetime, before: datetime, db: Session):
    return db.query(models.Employee).filter(models.Employee.birth >= after,
                                            models.Employee.birth <= before).all()


def get_employees_in_onboard_range(after: datetime, before: datetime, db: Session):
    return db.query(models.Employee).filter(models.Employee.onboard >= after,
                                            models.Employee.onboard <= before).all()
...
def update_employee(employee: schemas.Employee, db: Session):
    ...
    db.query(models.Employee). \
        ...
        update(jsonable_encoder(updated_employee))
    db.commit()
    return db.query(models.Employee).filter(models.Employee.id == updated_employee.id).first()
