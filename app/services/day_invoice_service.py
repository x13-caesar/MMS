...
def get_day_invoices_by_batch_process_id(batch_process_id: int, db: Session):
    return db.query(DayInvoice).filter(DayInvoice.batch_process_id == batch_process_id).all()
...
def get_day_invoices_in_work_date_range(after: datetime, before: datetime, db: Session):
    return db.query(DayInvoice).filter(DayInvoice.work_date >= after,
                                       DayInvoice.work_date <= before).all()


def get_day_invoices_by_employee_id_and_work_date_range(employee_id: int, after: datetime, before: datetime,
                                                        db: Session):
    return db.query(DayInvoice).filter(DayInvoice.employee_id == employee_id,
                                       DayInvoice.work_date >= after,
                                       DayInvoice.work_date <= before).all()


def get_unchecked_day_invoices_by_employee_id_and_work_date_range(employee_id: int, after: datetime, before: datetime,
                                                                  db: Session):
    return db.query(DayInvoice).filter(DayInvoice.employee_id == employee_id,
                                       DayInvoice.work_date >= after,
                                       DayInvoice.work_date <= before,
                                       DayInvoice.check_status == False).all()
...
def update_day_invoice(day_invoice: schemas.DayInvoice, db: Session):
    ...
    db.query(DayInvoice). \
        filter(DayInvoice.id == db_day_invoice.id). \
        update(json_day_invoice)
    db.commit()
...
def delete_day_invoice(day_invoice: schemas.DayInvoice, db: Session):
    db.query(DayInvoice). \
        filter(DayInvoice.id == day_invoice.id). \
