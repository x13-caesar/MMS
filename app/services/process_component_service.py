...
def get_process_component(process_component_id: int, db: Session):
    return db.query(models.ProcessComponent).filter(models.ProcessComponent.id == process_component_id).first()
...
def get_process_components_by_component_id(component_id: str, db: Session):
    return db.query(models.ProcessComponent).filter(models.ProcessComponent.component_id == component_id).all()
...
def update_process_component(process_component: schemas.ProcessComponent, db: Session):
    ...
    db.commit()
    return db.query(models.ProcessComponent).filter(models.ProcessComponent.id == updated_process_component.id).first()


def delete_process_component(process_component: schemas.ProcessComponent, db: Session):
    db.query(models.ProcessComponent). \
        filter(models.ProcessComponent.id == process_component.id). \
