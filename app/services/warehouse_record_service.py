...
def get_warehouse_record(warehouse_record_id: int, db: Session):
    return db.query(WarehouseRecord).filter(WarehouseRecord.id == warehouse_record_id).first()
...
def get_warehouse_records_by_component_id(component_id: str, db: Session):
    return db.query(WarehouseRecord).filter(WarehouseRecord.component_id == component_id).all()


def get_warehouse_records_by_batch_process_id(batch_process_id: int, db: Session):
    return db.query(WarehouseRecord).filter(WarehouseRecord.batch_process_id == batch_process_id).all()


def create_warehouse_record(warehouse_record: schemas.WarehouseRecordCreate, db: Session):
...
def update_warehouse_record(warehouse_record: schemas.WarehouseRecord, db: Session):
    ...
    db.commit()
    return db.query(WarehouseRecord).filter(WarehouseRecord.id == updated_warehouse_record.id).first()


def delete_warehouse_record(warehouse_record: schemas.WarehouseRecord, db: Session):
    db.query(WarehouseRecord). \
        filter(WarehouseRecord.id == warehouse_record.id). \
