...
def update_process(process: schemas.Process, db: Session):
    ...
    db.query(models.Process). \
        filter(models.Process.id == updated_process.id). \
        update(jsonable_encoder(updated_process))
    db.commit()
    return db.query(models.Process).filter(models.Process.id == updated_process.id).first()


def delete_process(process: schemas.Process, db: Session):
    db.query(models.Process). \
        filter(models.Process.id == process.id). \
